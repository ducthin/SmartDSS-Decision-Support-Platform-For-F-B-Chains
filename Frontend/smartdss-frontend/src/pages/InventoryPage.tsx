  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <label className="flex items-center gap-2 px-3 py-2 border border-[#DCC2A8] rounded-lg cursor-pointer hover:bg-[#FDF6EC] text-sm">
        ...
        <table className="w-full text-sm">
          <thead className="bg-[#FDF6EC]">
            <tr>
              ...
              <th className="text-left py-3 px-4 font-medium text-[#6D4C41]">Tồn kho</th>
              <th className="text-left py-3 px-4 font-medium text-[#6D4C41]">Đơn vị</th>
              <th className="text-left py-3 px-4 font-medium text-[#6D4C41]">Mức tối thiểu</th>
          ...
          <tbody>
            {inventory.map((inv) => {
              ...
              return (
                <tr key={inv.id} className={`border-t border-[#F1E4D6] ${isLow ? 'bg-red-50' : ''}`}>
                  <td className="py-3 px-4 font-medium">{inv.ingredientName}</td>
                  <td className="py-3 px-4">{inv.quantity}</td>
                  <td className="py-3 px-4 text-[#6D4C41]">{inv.unit}</td>
                  <td className="py-3 px-4 text-[#6D4C41]">{inv.minimumStock}</td>
            ...
            {inventory.length === 0 && <tr><td colSpan={6} className="py-8 text-center text-[#A1887F]">Chưa có nguyên liệu</td></tr>}
          </tbody>
        </table>
        {pageData && (
        ...
        <div className="space-y-4">
          <div>
            ...
            <input type="number" value={form.quantity || ''} onChange={(e) => setForm({ ...form, quantity: e.target.value === '' ? 0 : Number(e.target.value) })}
              className="w-full px-3 py-2 border border-[#DCC2A8] rounded-lg focus:ring-2 focus:ring-[#F4A825] outline-none" min={1} />
          </div>
          <div>
