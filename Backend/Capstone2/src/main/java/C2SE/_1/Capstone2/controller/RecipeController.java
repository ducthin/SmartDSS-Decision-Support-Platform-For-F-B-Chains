...
public class RecipeController {
    ...
    @PostMapping
    public ResponseEntity<ApiResponse<RecipeDTO>> createRecipe(@RequestBody RecipeDTO recipeDTO) {
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(ApiResponse.success(recipeService.createRecipe(recipeDTO)));
    }

    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<RecipeDTO>> updateRecipe(@PathVariable Long id, @RequestBody RecipeDTO recipeDTO) {
